...
-- Count user_id, user_first_touch_timestamp, email, updated fields where it is not null
...
-- MAGIC
-- MAGIC #This also returns the same value as the SQL count in the above cell
...
-- SELECT DISTINCT * FROM users_dirty also works
-- DISTINCT(*) might not be valid SQL syntax
...
SELECT max((email)) FROM users_dirty GROUP BY user_id;
-- return the email address that comes last lexicographically for each unique user id
...
-- does max(updated) makes sure we get the latest updated data from the table?
